  ...
  implicit object BlockCypherTransactionFormat extends RootJsonFormat[BlockCypherTransaction] {
    ...
    override def write(tx : BlockCypherTransaction) : JsValue = {
      ...
      val m : Map[String,JsValue] = Map(
        ...
        receivedKey -> JsString(tx.received.toString(dateTimePattern)),
        confirmedKey -> (if(tx.confirmed.isDefined) JsString(tx.confirmed.get.toString(dateTimePattern)) else JsNull),
